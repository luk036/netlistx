...
> **Given**: Graph $G = (V, E)$ with edge weights $w: E \to \mathbb{R}^+$
>
...
graph LR
    ...
    subgraph T["Partition V\S"]
        3((3)) --- 4((4))
    end
    1 -.- 3
    2 -.- 4
    linkStyle 2,3 stroke:#0f0
```
...
### But... Planar Graphs are Special 🗺️
...
>
> A cut is bipartite ⟺ it contains **no odd cycles**.

### The Duality Trick

```mermaid
flowchart LR
    subgraph Primal["Primal Planar Graph G"]
        direction TB
        P0((1)) --- P1((2))
        P1 --- P2((3))
        P2 --- P0
...
**Each face in G → vertex in G\***. Edge weight = shared primal edge weight.
...
```mermaid
flowchart TD
    A["Input: Planar Graph G"] --> B["Build Dual Graph G*"]
    B --> C["Identify Odd Faces<br/>(odd-length cycles)"]
    C --> D["Minimum Weight Perfect<br/>Matching on Odd Faces"]
    D --> E["Exclude Primal Edges<br/>on Matched Paths"]
    E --> F["Remaining Edges = Max Cut"]
    F --> G{Validate<br/>Bipartite?}
    G -->|Yes| H["✅ Done"]
    G -->|No| I["⚠ Bug!"]
    I --> A
```
...
```
  ...
  length = 3 ✔️       length = 4 ✘       length = 5 ✔️
```

---

## 7. Step 3: Minimum Weight Perfect Matching ⚖️
...
|-----------|-------|----------|
| `build_dual` | 45 | `std::map` edge-face mapping |
| `dijkstra` | 25 | Priority queue |
| `exact_mwpm` | 45 | DP over subsets |
| `greedy_mwpm` | 20 | Greedy 2-approx |
| `solve_hadlock_max_cut` | 100 | Orchestrator |
| `validate_max_cut` | 30 | BFS 2-colouring |

**C++ Design Decisions**:
- Templates + callables (not virtual dispatch) 🏎️
- `py::set<edge_t>` return type matching Python API
- Hash for `std::pair` via specialization
- `[[maybe_unused]]` for warning-free MSVC builds

---

## 11. Comparison: Python vs C++ 🆚

| Aspect | Python | C++ |
|--------|--------|-----|
| Face extraction | `nx.check_planarity()` | Caller-provided |
| Dual graph | NetworkX Graph | `vector<vector<DualEdge>>` |
| MWPM | `nx.algorithms.matching` | Custom DP/greedy |
| Type safety | Duck typing | `node_t` template param |
| Weight lookup | `G[u][v]['weight']` | `weight(u, v)` callable |
| Build system | pip | **xmake** (C++23) |
...
| MWPM (exact, n ≤ 18) | $O(n^2 2^n)$ | DP over subsets |
...
> "The maximum cut problem in planar graphs is solvable in polynomial time."
> — Hadlock, 1975

```mermaid
flowchart LR
    T["Theory (1975)"] --> P["Python (2026)"]
    P --> C["C++20 (2026)"]
    C --> V["Verified ✅"]
```
...
*Created with ❤️ using markdown + Mermaid*
